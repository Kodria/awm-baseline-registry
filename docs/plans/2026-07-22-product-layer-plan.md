...
- [ ] **Step 1: Escribir `SKILL.md`** adoptando la metodología brief-spec (traducir al inglés, conservar TODAS estas reglas — son el corazón y no se diluyen):
  ...
  - **Reglas no negociables** (heredadas de brief-spec, cada una con su redacción normativa): mandato de no-asunción como primera sección con lista exhaustiva de lo NO verificado; lenguaje calibrado en certeza ("verify in R0", nunca certeza sin fuente); procesos agnósticos de implementación; releases independientemente productivos con justificación de valor en una línea; R0 siempre descubrimiento solo-lectura; trazabilidad por IDs (N#, PR-#, RF-x.y, RNF-x.y, RNF-T.#, CA-x.y, DA-#) — sin `P#` (principios): no tiene sección de origen en el contrato de 12 secciones, se descarta por YAGNI en vez de inventar una; DAs en tabla con columnas "blocks" y "known positions"; secuencia de releases por valor de negocio con justificación escrita; fuera-de-alcance con la misma seriedad que el alcance.
...
- [ ] **Step 2: Escribir `references/brief-template.md`** — esqueleto completo del brief: frontmatter (bloque de Task 2) + las 12 secciones de R5.1 en orden (ver `skills/readiness-gate/references/brief-contract.md` para el orden exacto y la redacción de Users & context / Constraints), cada una con 1-2 líneas de guía inline (`<!-- guidance -->`) y ejemplos de ID. Reconstruida desde las reglas (la plantilla original no existe — hallazgo de Task 1).
...
- [ ] **Step 1: Escribir `SKILL.md`:**
  ...
  - **Termination:** informe portable con frontmatter (`mode: assessment`, R5.3, `readiness: n/a` — ver `skills/readiness-gate/references/brief-contract.md`); si los hallazgos derivan en trabajo → ofrecer encadenar a `product-brief` (R2.1).
  - **Esquema de IDs para `open_decisions` (gap detectado en revisión de Task 5 — cerrar aquí):** los hallazgos de assessment no son "decisiones abiertas" en el sentido de un brief (no bloquean un release ajeno) — son riesgos. Reusar `DA-#` únicamente para hallazgos que si derivan en un brief se convertirán en decisiones abiertas reales de ese brief (con su `blocks`); los demás hallazgos (riesgos que no bloquean nada, solo se documentan) NO llevan ID de `open_decisions` — viven solo en la tabla de hallazgos de la Fase 4, con su propia columna severidad/evidencia. No inventar un prefijo nuevo (nada de `F-#`/`R-#`): o es `DA-#` porque bloquea algo real, o no entra a `open_decisions` en absoluto.
