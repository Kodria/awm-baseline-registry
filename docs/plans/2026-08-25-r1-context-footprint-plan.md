...
| T1 | Scheduled | Trigger: after candidate is green and before review | Replace with exact candidate commit | Re-run the same `wc -c` definitions | Record exact observed closure and gross byte delta | Record actual dispatch count already produced by normal execution | Record targeted commands already required by Task 1 | Copy only provider-native fields; otherwise `unobservable` | Must remain zero added model calls | Must show candidate closure `<=28861` bytes or R1 fails. |
| T2 | Scheduled | Trigger: after per-task spec and code-quality reviews | Replace with reviewed candidate commit | Carry exact T1 definition | Record fixes and any reference overhead | Record actual normal review dispatches, including retries | Record review findings and reruns | Provider-native only | No measurement-only dispatch | Controller updates this row after review; it does not create another task/reviewer. |
| T3 | Scheduled | Trigger: after normal post-implementation QA | Replace with final QA commit | Record implementation-repo feedforward context | Record final exact closure | Record all normal cycle dispatches | Record sensor/test/QA evidence | Provider-native only | No measurement-only dispatch | Reports the real cost of building R1; it must not claim clean end-to-end savings. |
...
- Execution remains interactive; empirical `--verify-sensors` is not required by the unattended gate.
