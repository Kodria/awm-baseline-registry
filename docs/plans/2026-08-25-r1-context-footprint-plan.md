...
| T4 | Captured | `2026-08-25T10:21:44Z`, after the first normal development cycle with v3.7.0 installed | Registry `572d9e533f5498d0b3bd8033638ffea6e68ae0b3` (`v3.7.0`); project candidate `338c09a` / [agentic-workflow PR #128](https://github.com/Kodria/agentic-workflow/pull/128), fixing #118 | `exact`: observed product repo `AGENTS.md=32778`, `CONSTITUTION.md=30164`, `CLAUDE.md=4539`, total `67481`; source: `wc -c`. | `exact`: observed closure `11382` bytes (`3390 + 3152 + 2341 + 954 + 1140 + 405`), reduction `36721` bytes (76.34%) from T0; source: `wc -c`. | `exact`: 1 independent code-quality review dispatch; no measurement-only dispatch. | `exact`: focused regression suite 36 pass, CLI build pass, full CLI Jest suite pass, and `awm sensors run` `overall: pass`; rollback test was deliberately red with the detection branch removed, then restored green. | `unobservable`: provider-native input/output/cache/cost fields unavailable; no owner billing record supplied. | `exact`: zero measurement-only model calls; one normal review dispatch recorded separately. | Real cycle, not synthetic. #128 was open at collection. No billed-token or net-savings claim is valid without provider-reported/owner-supplied usage. Issue trace: [#126 comment](https://github.com/Kodria/agentic-workflow/issues/126#issuecomment-5409093869). |
