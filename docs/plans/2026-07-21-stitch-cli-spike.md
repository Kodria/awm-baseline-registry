...
| `create_design_system` | `npx -y @_davideast/stitch-mcp tool create_design_system -d '{"designSystem":{"displayName":"<name>","theme":{...}},"projectId":"<projectId>"}' -o json` | Por schema (+ 1 llamada en vivo accidental sin `designSystem`, ver advertencias) | `designSystem` (objeto `DesignSystem`, requiere `displayName` + `theme`) requerido; `projectId` opcional (vacío = asset global). |
