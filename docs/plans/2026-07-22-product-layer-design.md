...
- **R5.1** — THE cuerpo del brief SHALL seguir la metodología brief-spec: necesidad (N#), casuísticas de negocio, usuarios y contexto, restricciones, mandato de no-asunción con lista de lo NO verificado, procesos (PR-#), requisitos (RF-x.y/RNF-x.y) con redacción EARS-compatible, decisiones abiertas (DA-#) en tabla con columna "bloquea", fuera-de-alcance explícito, releases con valor independiente, riesgos. (Nota post-implementación: "usuarios y contexto" y "restricciones" se agregaron como secciones explícitas durante Task 2 al detectarse que G2/G5 del gate no tenían sección de origen; "casuísticas de negocio" se agregó en la revisión final de Task 12 al detectarse el mismo problema para G4 — ver `skills/readiness-gate/references/brief-contract.md`.)
- **R5.2** — THE sello `readiness` SHALL ser escrito exclusivamente por `readiness-gate`; las skills de modo no lo modifican.
- **R5.3** — THE informes de assessment y extracción SHALL usar el mismo frontmatter (con su `mode`) para ser recuperables por re-ingesta. (Aclaración post-Task 5: paridad de frontmatter únicamente — el cuerpo de estos informes NO sigue las 12 secciones de R5.1, que son de negocio y no tienen equivalente arquitectónico; `readiness` en estos modos toma el valor `n/a`, nunca `draft`/`ready` autoasignado. Ver `skills/readiness-gate/references/brief-contract.md`.)
