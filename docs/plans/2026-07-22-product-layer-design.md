...
- **R5.3** — THE informes de assessment y extracción SHALL usar el mismo frontmatter (con su `mode`) para ser recuperables por re-ingesta. (Aclaración post-Task 5: paridad de frontmatter únicamente — el cuerpo de estos informes NO sigue las 11 secciones de R5.1, que son de negocio y no tienen equivalente arquitectónico; `readiness` en estos modos toma el valor `n/a`, nunca `draft`/`ready` autoasignado. Ver `skills/readiness-gate/references/brief-contract.md`.)
