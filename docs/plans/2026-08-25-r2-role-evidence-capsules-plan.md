...
| T1 | Captured before review on implementation worktree; corrected after logic review | The initial `20510`/`97.14%` figure used a synthetic capsule and is invalidated. Candidate prefixes: implementer `3200`, specification `2100`, code quality `2036`, Track A `2986`, robustness `2959`, logic `2869`, tests `2801`; corpus-derived capsules `19421/19423/128775/163196/1685/1975/36438`; aggregate `389864` bytes (`45.72%` below T0, ceiling `430938`). Structural bytes only; not billed-token/cost/quota savings. | `7` normal frozen-topology dispatch shapes. | `0 / 0`; measurement-only model calls `0`. | R13 contract green; review found and closed the comparability defect. | Provider input/output/cache/cost `unobservable`. | Parent adds issue #126 checkpoint after review/closure. |
| T2 | Captured after normal implementation/specification/code-quality reviews; measurement corrected before QA | Candidate initial aggregate `389864` bytes, `45.72%` below T0; seven frozen-topology shapes; prefix/capsule detail remains in R13 output. | `10` normal dispatches: 4 implementer/correction, 4 specification reviews, 2 code-quality reviews; zero measurement-only dispatches. | `0 / 0`; no natural retrieval or fallback; corrections were normal review work. | Specification re-review `compliant`; code-quality re-review `0 critical / 0 important / 0 minor`; later logic review closed a measurement-comparability blocker and three important contract gaps. | Provider input/output/cache/cost `unobservable`. | Parent adds issue #126 checkpoint before final PR. |
