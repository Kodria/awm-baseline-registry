...
2. **Mockups:** for each `pending` screen, invoke the `impeccable` skill's `craft` sub-command, using `MASTER.md` tokens + the screen's Description as the brief, to produce a **static, self-contained HTML mockup**.
   - **Force the greenfield menu.** Per `craft.md`'s Step 0, `craft` only offers the "Single index.html" option when the working directory is empty — if it detects an existing framework (`package.json`, `astro.config.*`, etc., which most real projects have) it silently reuses that framework instead, defeating the point of a portable offline mockup. So run `craft` with its working directory pointed at a **fresh, empty scratch directory** created new for this screen, e.g. `.stitch/.scratch/<screen-slug>/` (never reused across screens), so its Step 0 `ls` check always sees an empty dir and offers the greenfield menu. From that menu, choose **"Single index.html."**
   - Once `craft` writes `index.html` in the scratch dir, copy it to `.stitch/designs/<screen-slug>.html` and delete the scratch dir.
   - **Stop at the artifact — do not run the rest of craft's pipeline.** `craft.md`'s Step 4 (Build to Production Quality), Step 5 (Iterate Visually), and Step 6 (Present) assume a real, running feature: production-hardening checklist, in-browser inspection loop, build pipeline. None of that applies to a single static mockup file. Use `craft` only far enough to get the written `index.html` from its Step 0 (framework choice) + Step 1 (shape brief, run internally) + an initial pass of markup/CSS — then stop. This skill's own approval loop (next bullet) stands in for craft's Step 5/6.
   - **Present without a screenshot.** No PNG exists yet at this point (PNG capture is point 3 below, and only after approval) — "presenting" a screen here means describing the generated HTML or opening the file for the user to view directly, not showing a screenshot. If the user requests changes, re-invoke `craft` (same scratch-dir approach) with their feedback to adjust the HTML — this replaces `edit_screens` from the online flow. Repeat until approved, one screen at a time, before moving to the next `pending` screen.
3. **PNG:** check your tool list for browser-automation tools (e.g. `browser_navigate`, `browser_resize`, `browser_take_screenshot` — the Playwright MCP tool family), same concrete-check style as Step 0. If present: navigate to `file://<abs-path>/.stitch/designs/<slug>.html`, resize to the device width (1600 desktop / 390 mobile), screenshot → `.stitch/designs/<slug>.png`. If no such tools are present, record `png: n/a (offline, no browser)` in the Artifacts column — an explicit degradation, never a silent one.
