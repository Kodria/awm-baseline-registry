...
**As a post-implementation-qa lens:** the dispatching controller wraps this skill's Steps 1-4 comparison procedure with QA's standard Output Format and ledger instructions (see `post-implementation-qa/deep-review-prompt.md`). Each element with status `missing`/`diverged` becomes one finding: `"track": "B"`, `"lens": "design-fidelity"`, `"severity"` mapped from this skill's high/medium/low scale to blocker/important/minor respectively, `"title"` the element name, `"detail"` the divergence description, `"evidence"` the file:line-equivalent (design artifact path + implementation screenshot path), `"reference"` the screen name. Elements marked `present` are not findings. A `CONVERGED` verdict with zero findings still emits `{"findings": [], "summary": "..."}` per the standard empty-case format.
