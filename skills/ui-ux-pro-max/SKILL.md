...
The search script lives inside this skill's own directory, not the project directory. Resolve the skill root once per session (first match wins — AWM installs are directory symlinks, so executing through them works; no `readlink` needed). The two relative candidates resolve against whatever directory the agent is currently in (typically the project root) — that's why the `$HOME/...` roots are checked first, before any cwd-dependent path:
...
```bash
python3 "$UIPRO/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python3 "$UIPRO/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python3 "$UIPRO/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python3 "$UIPRO/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 "$UIPRO/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python3 "$UIPRO/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python3 "$UIPRO/scripts/search.py" "<keyword>" --stack <stack>
```
...
python3 "$UIPRO/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python3 "$UIPRO/scripts/search.py" "search loading animation" --domain ux

# Step 4: stack guidelines
python3 "$UIPRO/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
