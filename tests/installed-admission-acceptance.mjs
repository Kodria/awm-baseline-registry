...
import { readCliCertification } from '../scripts/cli-certification.mjs';
...
export async function runInstalledAdmissionAcceptance(bin, root, { negativesOnly = false, publishedRemote = false } = {}) {
  ...
  try {
    ...
    // The registry declares a FLOOR, not the exact version CI must run. The
    // version actually under test is whatever CI resolved from npm and declared
    // in AWM_INSTALLED_CLI_VERSION — see Kodria/agentic-workflow#164.
    const { floor, negativeControls } = readCliCertification(root);
    ...
    process.stderr.write(`R16 provenance=${publishedRemote ? 'published-remote' : 'local-git-fixture'} candidate=${sourceCommit} installed=${installedCommit} cli=${actual} floor=${floor}; fixture transport is not public release evidence\n`);
    ...
    if (!negativesOnly) assert.ok(oldBin, `installed acceptance must include the unmodified published CLI ${negativeControls.admissionCli} negative control`);
    if (oldBin) {
      assert.equal(ok(oldBin, ['--version']), negativeControls.admissionCli);
      ...
      assert.match(compatibility.detail, new RegExp(`baseline requires CLI >= ${floor.replace(/\./g, '\\.')}`));
    ...
    if (actual === negativeControls.admissionCli) assertIncompatible(floor, actual);
    assertIncompatible('99.0.0', actual);
    if (negativesOnly) return { kind: 'prerelease-negative-controls', actual, custody: 'journal-absent' };
    assert.ok(satisfiesFloor(actual, floor), `matched installed acceptance requires a CLI at or above the registry floor ${floor}; got ${actual}`);
    // "Prerelease is not a PASS" was previously enforced by demanding the CLI
    // equal minCliVersion, which forced the floor to track every publish. The
    // claim is kept by making CI declare the exact npm-resolved version it
    // installed: a locally compiled binary cannot match it.
    const declaredInstalled = process.env.AWM_INSTALLED_CLI_VERSION;
    assert.ok(declaredInstalled, 'matched installed acceptance must declare AWM_INSTALLED_CLI_VERSION — the exact published version it installed; prerelease is not a PASS');
    assert.equal(actual, declaredInstalled, 'the CLI under test must be exactly the published version CI resolved and installed');
